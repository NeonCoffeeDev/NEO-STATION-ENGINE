/*
 * ps2_pad - Neon Coffee on the PlayStation 2
 *
...
int main(void)
{
    ...
    printf("ps2_pad: Neon Coffee, PlayStation 2\n");
    printf("ps2_pad: d-pad moves, X recolours, L1/R1 resize, START resets\n");
    ...
    if (!have_pad)
        printf("ps2_pad: no controller in port 1 -- the box will just sit there\n");
    ...
    while (1) {
        ...
        if ((frames++ % 60) == 0)
            printf("ps2_pad: frame %d  box %d,%d size %d\n", frames, x, y, size);
    }
