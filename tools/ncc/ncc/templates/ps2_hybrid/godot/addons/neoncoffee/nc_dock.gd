...
func _find_project_dir() -> String:
	...
	if FileAccess.file_exists(parent.path_join("CMakeLists.txt")) or FileAccess.file_exists(parent.path_join("nc.json")):
		return parent
	return ""
...
func _on_export() -> bool:
	...
	if root.has_method("export_project"):
		var problem: String = root.export_project()
		_say("VN exported; build/check validates references and budgets." if problem.is_empty() else problem, GREEN if problem.is_empty() else RED)
...
func _on_check() -> void:
	var root := EditorInterface.get_edited_scene_root()
	if root != null and root.has_method("export_project") and not _on_export():
		return
